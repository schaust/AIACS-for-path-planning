function [pathlength,pathsequence,OptimalPath] = ASTARPATH(StartX,StartY,MAP,GoalX,GoalY,Connecting_Distance)
[Height,Width]=size(MAP);            %Height and width of matrix
GoalRegister=zeros(Height,Width);
GoalRegister(GoalY,GoalX)=1;
...
for i = 1:size(OptimalPath,1)
    temp = OptimalPath(i,2) +(OptimalPath(i,1)-1) * Width;
    pathsequence = [pathsequence;temp];
end
pathlength =0;
for i = 1:size(OptimalPath,1)-1
    pathlength = pathlength + sqrt(sum((OptimalPath(i+1,:) - OptimalPath(i,:)).^2));    
end
end     
